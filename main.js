...
function updateChapterReturn() {
  if (!chapter2Content || !chapter2Return) return;

  const rect = chapter2Content.getBoundingClientRect();
  const isInChapterContent = rect.top < window.innerHeight * 0.72 && rect.bottom > window.innerHeight * 0.2;
  chapter2Return.classList.toggle("is-visible", isInChapterContent);
}
